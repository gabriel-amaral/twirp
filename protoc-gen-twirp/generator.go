...
func (t *twirp) generateUtils() {
	...
	t.P(`    return `, t.pkgs["twirp"], `.InternalError(msg).WithMeta("status_code", strconv.Itoa(statusCode))`)
	t.P(`      .WithMeta("body", string(respBodyBytes))`)
	t.P(`  }`)
	t.P(``)
	t.P(`  twerr := `, t.pkgs["twirp"], `.NewError(errorCode, tj.Msg).WithMeta("status_code", strconv.Itoa(statusCode))`)
